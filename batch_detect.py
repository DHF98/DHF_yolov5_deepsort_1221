...
# 检测参数
parser.add_argument('--weights', default=r"best5.0.pt", type=str, help='weights path')
parser.add_argument('--source', default=r"D:\DHF的小地盘\丁海峰学习材料\项目\其他\2023黄浦江\客船精细化管理\3.0 精准错时提质增效\黄浦海事数据\0701-0706视频数据\5-南浦大桥", type=str, help='img or video(.mp4)path')
    ...
    def __call__(self, image: np.ndarray):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            for t in online_targets:
                ...
                point_list.append([self.names[int(cls)], int(tlwh[0] + tlwh[2] / 2), int(tlwh[1] + tlwh[3] / 2), int(tid)])
...
if __name__ == '__main__':
    ...
    model = Detector(device=device, model_path=opt.weights, imgsz=opt.imgsz, conf=opt.conf_thre,
                     iou=opt.iou_thre, merge_nms=opt.merge_nms)

    # 遍历文件夹中的所有 mp4 文件
    video_files = [f for f in os.listdir(opt.source) if f.endswith('.mp4')]

    for video_file in video_files:
        video_path = os.path.join(opt.source, video_file)
        ...
        output_video_name = os.path.basename(video_file).rsplit('.', 1)[0] + "_out.mp4"
        outVideo = cv2.VideoWriter(os.path.join(opt.save, output_video_name), fourcc, fps, size)
        ...
        while True:
            ...
            # 确保文本文件的保存路径是有效的
            txt_file_path = os.path.join(save_txt_folder,
                                         f'{os.path.basename(video_file).rsplit(".", 1)[0]}_{frame_id}.txt')
            with open(txt_file_path, 'w') as outtxt:
                for point in point_list:
                    label, centerx, centery, tid = point[0], point[1], point[2], point[3]
                    outtxt.write(f'{label} {centerx} {centery} {tid}\n')
